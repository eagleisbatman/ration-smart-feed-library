#!/usr/bin/env python3
"""
Feed Formulation Backend - Production Deployment Script (New Server)

This script automates the deployment of the Feed Formulation Backend to the new AWS production server.
It handles SSH connection using PEM file, Docker operations, and provides rollback capability.

Usage: 
    python scripts/deploy_to_production_new.py

Features:
- SSH key-based authentication with PEM file
...
# Configuration for NEW Production Server
SERVER_CONFIG = {
    'host': 'ec2-18-60-203-199.ap-south-2.compute.amazonaws.com',
    'username': 'ubuntu',
    'ssh_key_path': '/Users/satishchandra/ucdapp/aws_instance/hyderabad-key.pem',
...
DEPLOYMENT_STEPS = [
    {
        ...
        'command': 'echo "Connected to production server"',
        'rollback': None
    },
    {
        'name': 'Navigate to project directory',
        'command': f'cd {SERVER_CONFIG["project_dir"]} && pwd',
        'rollback': None
    },
    {
        'name': 'Pull latest code from GitHub v3.0 branch',
        'command': None,  # Will be set dynamically with credentials
        'rollback': f'cd {SERVER_CONFIG["project_dir"]} && git reset --hard HEAD~1'
    },
    {
        'name': 'Pause for .env file modification',
        'command': None,  # Interactive step - no command needed
        'rollback': None,
        'interactive': True,
    ...
    },
    {
        ...
        'command': f'docker stop {SERVER_CONFIG["container_name"]} || echo \'No existing container to stop\'',
        'rollback': f'docker start {SERVER_CONFIG["container_name"]}'
    },
    {
        'name': 'Remove existing container',
        'command': f'docker rm {SERVER_CONFIG["container_name"]} || echo \'No existing container to remove\'',
        'rollback': None
    },
    {
        'name': 'Remove existing image',
        'command': f'docker rmi {SERVER_CONFIG["docker_image"]} || echo \'No existing image to remove\'',
        'rollback': None
    },
    {
        'name': 'Build new Docker image',
        'command': f'cd {SERVER_CONFIG["project_dir"]} && docker build -t {SERVER_CONFIG["docker_image"]} .',
        'rollback': None
    },
    {
        'name': 'Run new container',
        'command': f'cd {SERVER_CONFIG["project_dir"]} && docker run -d --name {SERVER_CONFIG["container_name"]} -p {SERVER_CONFIG["port"]}:{SERVER_CONFIG["port"]} --env-file .env {SERVER_CONFIG["docker_image"]}',
        'rollback': f'docker stop {SERVER_CONFIG["container_name"]} && docker rm {SERVER_CONFIG["container_name"]}'
    },
    {
...
class DeploymentLogger:
    ...
    def __init__(self, log_file: str = None):
        if log_file is None:
            ...
            log_file = f"production_deployment_log_{timestamp}.log"
...
class DeploymentManager:
    ...
    def handle_interactive_confirmation(self, step: dict) -> bool:
        """Handle interactive confirmation for deployment steps"""
        self.logger.info("⏸️  PAUSED: Interactive step - Manual intervention required")
        self.logger.info("📝 Instructions:")
        self.logger.info("   1. SSH into the server if needed: ssh -i /Users/satishchandra/ucdapp/aws_instance/hyderabad-key.pem ubuntu@ec2-18-60-203-199.ap-south-2.compute.amazonaws.com")
        self.logger.info("   2. Navigate to the project directory: cd feed-formulation-be")
        self.logger.info("   3. Modify the .env file as needed: nano .env (or vim .env)")
        self.logger.info("   4. Save the file and exit the editor")
        self.logger.info("   5. Return to this terminal and type 'yes' to continue deployment")
        self.logger.info("")
        
        while True:
            try:
                confirmation = input("🔐 Type 'yes' to continue deployment: ").strip().lower()
                if confirmation == 'yes':
                    self.logger.info("✅ Confirmation received. Continuing deployment...")
                    return True
                else:
                    self.logger.warning(f"⚠️  Invalid input '{confirmation}'. Please type 'yes' to continue.")
            except KeyboardInterrupt:
                self.logger.error("❌ Deployment cancelled by user")
                return False
            except EOFError:
    ...
    def run_deployment(self) -> bool:
        ...
        self.logger.info("🚀 Starting Feed Formulation Backend Deployment to NEW Production Server")
        self.logger.info(f"📋 Target Server: {self.config['host']}")
        ...
        try:
            ...
            for i, step in enumerate(DEPLOYMENT_STEPS, 1):
                ...
                if step.get('interactive', False):
                    success = self.handle_interactive_confirmation(step)
                    if not success:
                        self.logger.error("❌ Interactive step cancelled by user")
                        return False
                # Handle special case for git pull with credentials
                elif step['name'] == 'Pull latest code from GitHub v3.0 branch':
                    github_creds = self.get_github_credentials()
                    # Set up git credentials and pull
                    git_command = f'''cd {self.config['project_dir']} && 
                ...
                if success:
                    ...
                    if not step.get('interactive', False):
    ...
    def health_check(self) -> bool:
        ...
        success, stdout, stderr = self.deployer.execute_command('docker ps --filter name=feed-formulation-be --format "table {{.Names}}\t{{.Status}}"')
...
def main():
    """Main function"""
    parser = argparse.ArgumentParser(description='Deploy Feed Formulation Backend to NEW Production Server')
    ...
    if args.dry_run:
        ...
        for i, step in enumerate(DEPLOYMENT_STEPS, 1):
            if step['name'] == 'Pull latest code from GitHub v3.0 branch':
                print(f"{i}. {step['name']}: git pull origin v3.0 (with GitHub credentials)")
            elif step.get('interactive', False):
                print(f"{i}. {step['name']}: [INTERACTIVE] - Manual .env file modification required")
            else:
        ...
        print(f"   🔐 SSH Key: {SERVER_CONFIG['ssh_key_path']}")
        print("\n⏸️  Interactive Step:")
        print("   📝 Step 4: Manual .env file modification - Type 'yes' to continue")
        return
    ...
    if success:
        ...
        print(f"🌐 Application URL: http://{SERVER_CONFIG['host']}:{SERVER_CONFIG['port']}")
        sys.exit(0)
    else:
        print("\n❌ DEPLOYMENT FAILED!")
