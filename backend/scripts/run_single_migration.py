...
def main():
    """Main migration runner for single file."""
    print("🗃️  Feed Formulation - Single Migration Runner")
    print("=" * 50)
    ...
    # Define the specific migration file
    migration_file = "031_add_fd_category_id_to_feeds.sql"
    migration_description = "Add fd_category_id column to feeds table"
    ...
    if sql_content is None:
        print(f"❌ Failed to read {migration_file}")
        return False
    
    # Execute migration
    success = execute_migration(engine, sql_content, migration_file)
    if not success:
    ...
    print("\n📋 Verification Steps:")
    print("   1. Check if column exists: \\d feeds")
    print("   2. Verify foreign key constraint")
    print("   3. Test the bulk upload API")
